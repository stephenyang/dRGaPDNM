function [result] = HtGaP_DNM_batchGibbs(Adj, options)

%%
% dependent relational gamma process dynamic model
...
for iter = 1:maxiter
    ...
    for t = 1:T
        theta_KK_t = phi(:, :, t)' * bsxfun(@minus, sum(phi(:, :, t), 1), phi(:, :, t));
        %%
        p_n_k_dot_dot_t = sum(m_n_k_dot_dot(:, :, t), 2);
        p_n_k_dot_dot_t = p_n_k_dot_dot_t./sum(p_n_k_dot_dot_t);
        pos_ind = p_n_k_dot_dot_t > 1e-2;
...
%     end
    ...
    for k= randperm(K)
        kinds = Km{k};
        inv_sigma = diag(invSigma(:, k));
        Sigma = (inv_sigma + kinds' * diag(b_tilde_KT(k, :)) * kinds + jitter * eye(L + 1));
        Mu = Sigma\(kinds'*(b_KT(k, :)-0.5)');
        W(:, k) = mvnrnd(Mu', inv(Sigma))';
        % b_tilde_KT(k, :) = pgdraw(kinds * W(:, k));
        b_tilde_KT(k, :) = PolyaGamRnd_Gam(1,kinds * W(:, k),5);
    end
    ...
    %% P_n_k_t :: backward
            ...
            %%
            if options.samp_c_n == 1
            for i = 1:N
                c_n(i) = randg(1e-2 + sum(phi_0(i, :)) + sum(sum(phi(i, :, 1:(T-1)))))./(1e-2 + sum(sum(phi(i, :, :))));
            end
            end
    ...
    if mod(iter, 10) == 0
        ...
        if options.display
            ...
            for t = 1:slices
                figure(123);
                if re_order== 1
                    subplot(slices, 4, (t-1)* 4 + 1); imagesc(ATrain{t_ind(t)}(Rankdex, Rankdex));
                    subplot(slices, 4, (t-1)* 4 + 2); imagesc(ProbAve{t_ind(t)}(Rankdex, Rankdex));
                    subplot(slices, 4, (t-1)* 4 + 3); imagesc(log(phi(Rankdex, rdex, t_ind(t)) * lambda_KKT(rdex, rdex, t_ind(t)) + 1e-2));
                    subplot(slices, 4, (t-1)* 4 + 4); imagesc(log(lambda_KKT(rdex, rdex,t_ind(t)) + 1e-2));
                else
                    % if iter == 100
                    subplot(slices, 4, (t-1)* 4 + 1); imagesc(Adj{t_ind(t)});
                    % end
                    subplot(slices, 4, (t-1)* 4 + 2); imagesc(ProbAve{t_ind(t)});
                    subplot(slices, 4, (t-1)* 4 + 3); imagesc(log(phi(:,:, t_ind(t)) * lambda_KKT(:,:, t_ind(t)) + 1e-2));
                    subplot(slices, 4, (t-1)* 4 + 4); imagesc(log(lambda_KKT(:,:,t_ind(t)) + 1e-2));
                end
