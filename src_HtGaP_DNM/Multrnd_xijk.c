...
void Multrnd_Matrix(double *m_i_dot_k,  double *m_dot_j_k, double *PhiU, double *PhiV, double *r, mwIndex *ir, mwIndex *jc, double *pr,  mwSize Nsize, mwSize Ksize, mwSize Dsize double *prob_cumsum) 
{    
        ...
        if (starting_row_index == stopping_row_index)
            ...
            for (current_row_index =  starting_row_index; current_row_index<stopping_row_index; current_row_index++) {
                ...
                for (cum_sum=0, k=0; k<Ksize; k++){
                        cum_sum += PhiU[i+ k*Nsize]*PhiV[j+ k*Dsize]*r[k];
...
{
    double *m_i_dot_k, *m_dot_j_k, *m_dot_dot_k, *PhiU, *PhiV *r;
    double  *pr, *prob_cumsum;
    mwIndex *ir, *jc;
    mwIndex Nsize, Dsize, Ksize;
    ...
    plhs[2] = mxCreateDoubleMatrix(Ksize,Dsize,mxREAL);
    m_i_dot_k = mxGetPr(plhs[0]);
    m_dot_j_k = mxGetPr(plhs[2]);
    
    prob_cumsum = (double *) mxCalloc(Ksize,sizeof(double));

    Multrnd_Matrix(m_i_dot_k,  m_dot_j_k, PhiU, PhiV, r, ir, jc, pr, Nsize, Ksize, Dsize,  prob_cumsum);
    mxFree(prob_cumsum);
}