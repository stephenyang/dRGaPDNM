...
%%
% gamma process dynamic relational model
...
%             end
            ...
            if mod(iter, 10) == 0
                ...
                for t = 1:slices
                    subplot(slices, 4, (t-1)* 4 + 1);
                    imagesc(BTrain{t_ind(t)}+BTrain{t_ind(t)}');
                    subplot(slices, 4, (t-1)* 4 + 2);
                    imagesc(ProbAve{t_ind(t)});
                    subplot(slices, 4, (t-1)* 4 + 3);
                    imagesc(log(phi(:,:,t_ind(t)) * lambda_KK + 1e-2));
                    subplot(slices, 4, (t-1)* 4 + 4);
                    bar(r_K./sum(r_K));
                end
                drawnow;
                end
